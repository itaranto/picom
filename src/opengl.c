...
bool glx_init_rounded_corners(session_t *ps) {
...

	{
		...
		static const char *FRAG_SHADER_ROUND_CORNERS =
		    ...
		    ");\n"
		    "  //vec4 c = mix( vec4(1.0,0.0,0.0,1.0), vec4(0.0,1.0,0.0,0.0), "
		    "smoothstep(0.0,1.0,b) );\n"
		    "\n"
		    "  //if ( c == vec4(0.0,1.0,0.0,0.0) ) discard; else gl_FragColor = "
		    "vec4( c );\n"
		    "  //gl_FragColor = vec4( c );\n"
...
bool glx_round_corners_dst(session_t *ps attr_unused, int dx, int dy, int width, int height,
                           float z attr_unused, float cr, GLfloat factor_center attr_unused,
...

	{
		...
		P_PAINTREG_START(crect) {
			...
			// %f, %f, %f", 	texfac_x, texfac_y, rx, ry, rxe, rye, rdx, rdy,
			//rdxe, rdye);
...
#ifdef DEBUG_GLX
		        log_debug("Rounded corner Pass: %d, %d, %.2f -> %d, %d, %d, %d",
