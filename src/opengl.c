...
bool glx_init_rounded_corners(session_t *ps) {
...

	{
		...
		static const char *FRAG_SHADER_PREFIX =
		    ...
		    "void main()\n"
		    "{\n"
		    "  vec4 col = %s(tex_scr, vec2(gl_TexCoord[0].x, gl_TexCoord[0].y)) "
		    "* factor_center;\n"
		...
		glx_round_pass_t *ppass = &ps->psglx->round_passes[0];
		{
			...
			if (!shader_str) {
				log_error(
				    "Failed to allocate %zd bytes for shader string.", len);
				return false;
			}
			...
			if (!ppass->frag_shader) {
				log_error("Failed to create rounded corners fragment "
				          "shader.");
				free(extension);
				free(lc_numeric_old);
				return false;
...
bool glx_round_corners_dst(session_t *ps, const glx_texture_t *ptex attr_unused, int dx,
                           int dy, int width, int height, float z, float cr,
                           GLfloat factor_center, const region_t *reg_tgt attr_unused,
...
#ifdef DEBUG_GLX
		        log_debug("Rounded corner Pass: %f, %f, %f, %f -> %f, %f, %f, %f",
		                  rx, ry, rxe, rye, rdx, rdy, rdxe, rdye);
#endif

		        // log_warn("Rounded corner Pass:  %f, %f, %f, %f, %f, %f -> %f,
		        // %f, %f, %f", 	texfac_x, texfac_y, rx, ry, rxe, rye, rdx,
rdy,
		        //rdxe, rdye);
...
#ifdef DEBUG_GLX
		        log_debug("Rounded corner Pass: %d, %d, %.2f -> %f, %f, %f, %f",
crect.x1, crect.y1, z, rdx, rdy, rdxe, rdye); #endif
 ...
 */
bool glx_render(session_t *ps, const struct managed_win *w attr_unused,
