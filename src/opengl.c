...
bool glx_init_rounded_corners(session_t *ps) {
...

	{
		...
		static const char *FRAG_SHADER_ROUND_CORNERS_PRE =
		    "  // colorize (red / black )\n"
		    "  //vec3 c = mix( vec3(col.r,col.g,col.b), vec3(0.0,1.0,0.0), "
		    "smoothstep(0.0,1.0,b) );\n"
		    "  vec4 c = mix( col_scr, vec4(0.0,0.0,0.0,0.0), "
		    "smoothstep(0.0,1.0,b) );\n"
		    "  //vec4 c = mix( vec4(1.0,1.0,1.0,1.0), vec4(0.0,0.0,0.0,0.0), "
		    "smoothstep(0.0,1.0,b) );\n"
		    "  //vec4 c = mix( vec4(1.0,0.0,0.0,0.0), vec4(0.0,1.0,0.0,0.0), "
		    "smoothstep(0.0,1.0,b) );\n"
		    "\n"
		...
		static const char *FRAG_SHADER_ROUND_CORNERS_POST =
		    "  // colorize (red / black )\n"
		    "  //vec4 c = mix( vec4(0.0,0.0,0.0,0.0), vec4(1.0,1.0,1.0,1.0), "
		    "smoothstep(0.0,1.0,b) );\n"
		    "  vec4 c = mix( vec4(1.0,1.0,1.0,1.0), vec4(0.0,0.0,0.0,0.0), "
		    "smoothstep(0.0,1.0,b) );\n"
		    "  //vec4 c = mix( vec4(1.0,0.0,0.0,1.0), vec4(0.0,1.0,0.0,1.0), "
		    "smoothstep(0.0,1.0,b) );\n"
		    "\n"
		    "  //if ( c == vec4(0.0,0.0,0.0,0.0) ) discard; else\n"
		...
		if (!glx_init_frag_shader_corners(&ps->psglx->round_passes[0], FRAG_SHADER_PREFIX,
		                                  FRAG_SHADER_ROUND_CORNERS_PRE,
		...
		if (!glx_init_frag_shader_corners(&ps->psglx->round_passes[1], FRAG_SHADER_PREFIX,
		                                  FRAG_SHADER_ROUND_CORNERS_POST,
		                                  extension, sampler_type, texture_func)) {
...
bool glx_round_corners_dst(session_t *ps, const glx_texture_t *ptex attr_unused, int shader_idx,
	...
	log_warn("dxy(%d, %d) wh(%d %d) rwh(%d %d)", dx, dy, width, height,
...

	{
...

		{
			...
			// This is all weird, but X Render is using premultiplied ARGB
			// format, and we need to use those things to correct it. Thanks
			// to derhass for help.
			// glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
			...
			GL_ONE_MINUS_SRC_COLOR); glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE1_RGB,
			GL_PRIMARY_COLOR); glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND1_RGB,
			GL_SRC_COLOR);
		...
		// bind_sampler_to_unit_with_texture(ppass->prog, "tex_scr", 1, tex_tgt,
		// pbc->textures[1]); if (ptex) {
		// bind_sampler_to_unit_with_texture(ppass->prog, "tex_bg", 1,
...
#ifdef DEBUG_GLX
		                log_debug("Rounded corner Pass: %d, %d, %.2f -> %f, %f,
%f, %f", crect.x1, crect.y1, z, rdx, rdy, rdxe, rdye); #endif

		                log_warn("Rounded corner Pass: %d, %d, %.2f -> %f, %f, %f,
%f", crect.x1, crect.y1, z, rdx, rdy, rdxe, rdye);
		...
		} else*/
		{
			P_PAINTREG_START(crect) {
				...
				log_warn("Rect %d (i:%d): %f, %f, %f, %f -> %f, %f, %f, "
				         "%f",
				         ri, ptex ? ptex->y_inverted : -1, rx, ry, rxe,
...
bool glx_round_corners_dst2(session_t *ps, const glx_texture_t *ptex, int shader_idx,
	...
	log_warn("dxy(%d, %d) wh(%d %d) rwh(%d %d) z(%.2f)", dx, dy, width, height,
	         ps->root_width, ps->root_height, z);
 ...
 */
bool glx_render(session_t *ps, struct managed_win *w attr_unused,
                const glx_texture_t *ptex, int x, int y, int dx, int dy, int width,
                int height, int z, double opacity, bool argb, bool neg, int cr attr_unused,
	...
	if (opacity < 1.0 || argb || cr > 0) {
		...
		if (cr == 0) {
			...
			glx_round_corners_dst2(
			    ps, ptex, 1, dx, dy, to_u16_checked(w->widthb),
			    to_u16_checked(w->heightb), (float)ps->psglx->z - 0.5f,
			    (float)cr, reg_tgt, &w->glx_round_cache);
			glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
			// glDisable(GL_BLEND);
