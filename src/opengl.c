...
bool glx_init_rounded_corners(session_t *ps) {
...

	{
		...
		static const char *FRAG_SHADER_ROUND_CORNERS_PRE =
		    ...
		    "  //vec4 c = mix( col_scr, vec4(0.0,1.0,0.0,0.0), "
		    "smoothstep(0.0,1.0,b) );\n"
		    "  vec4 c = mix( vec4(0.0,0.0,0.0,0.0), col_scr, "
		    "smoothstep(0.0,1.0,b) );\n"
...
bool glx_bind_texture(session_t *ps attr_unused, glx_texture_t **pptex, int x, int y,
	...
	if (ptex && ptex->texture && (ptex->width != width || ptex->height != height)) {
		log_warn("Windows size changed old_wh(%d %d) new_wh(%d %d)", ptex->width,
...
bool glx_round_corners_dst0(session_t *ps, const glx_texture_t *ptex attr_unused, int shader_idx,
...

	{
		...
		// bind_sampler_to_unit_with_texture(ppass->prog, "tex_scr", 0, tex_tgt,
		// tex_scr);
		if (ptex) {
			log_warn("ptex: %p wh(%d %d) %d %d", ptex, ptex->width,
			         ptex->height, ptex->target, ptex->texture);
			bind_sampler_to_unit_with_texture(ppass->prog, "tex_bg", 0,
